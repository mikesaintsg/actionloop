...
ActionLoop delivers the **Predictive Procedural Action Loop System (PPALS)**: a unified architecture that combines deterministic workflow rules with adaptive, data-driven predictions.  It guides users through complex workflows by suggesting valid next actions ranked by learned behavior patterns.

### Package Boundaries

| Responsibility                    | Owned By                                      |
|-----------------------------------|-----------------------------------------------|
| Valid transition rules            | `@mikesaintsg/actionloop` (ProceduralGraph)   |
| Learned usage weights             | `@mikesaintsg/actionloop` (PredictiveGraph)   |
| Transition recording & prediction | `@mikesaintsg/actionloop` (WorkflowEngine)    |
| Graph construction API            | `@mikesaintsg/actionloop` (WorkflowBuilder)   |
| Static graph validation           | `@mikesaintsg/actionloop` (WorkflowValidator) |
| Pattern detection & analysis      | `@mikesaintsg/actionloop` (WorkflowAnalyzer)  |
| UI components for recommendations | `@mikesaintsg/components` (separate package)  |
| Visual workflow editor            | `@mikesaintsg/platform` (separate package)    |
...
interface ProceduralGraphInterface extends Destroyable {
	// Accessors
...
// PredictiveGraph: Dynamic weight overlay
interface PredictiveGraphInterface extends Destroyable {
	// Accessors
	...
	// Subscriptions
	onWeightUpdate(callback: WeightUpdateCallback): Unsubscribe
	onDecay(callback: DecayCallback): Unsubscribe
}

// WorkflowEngine: Runtime orchestration
interface WorkflowEngineInterface extends Destroyable {
	// Core methods
...
const predictive = createPredictiveGraph(procedural, {
	decayAlgorithm: 'ewma',
	decayFactor: 0.9,
	preloadRecords: historicalData, // Optional cold-start data
})
...
// => ['projects', 'settings'] ranked by learned weights
````
...
// WorkflowValidator: Static analysis
...
const bottlenecks = analyzer.findBottlenecks({ trafficThreshold: 20 })
const opportunities = analyzer.findAutomationOpportunities({ minRepetitions: 5 })
...
// Each actor maintains separate weight tracks
predictive.getWeight('login', 'dashboard', 'user')      // User-initiated
predictive.getWeight('dashboard', 'timeout', 'system')  // System-triggered
predictive.getWeight('report', 'export', 'automation')  // Bot/LLM-initiated
````
...
// Linear decay: constant weight loss over time
const predictive = createPredictiveGraph(procedural, {
	decayAlgorithm: 'linear',
})

// No decay: weights accumulate indefinitely
const predictive = createPredictiveGraph(procedural, {
	decayAlgorithm: 'none',
})
````
...
ActionLoop integrates with the @mikesaintsg ecosystem through composition:
